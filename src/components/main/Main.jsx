...
function Main({ selectedBrand, selectedColor, sortBy }) {
  ...
  return (
    <main className="main flex-1 h-screen overflow-y-auto text-black p-4">
      ...
      <ul className="products">
        {sortedProducts.map((p) => (
          <li className="products-card p-4 rounded-lg" key={p.id}>
            <Link to={`/product/${p.id}`}>
              ...
              <div className="flex space-x-2 mt-2">
                {p.color_options.map((color, index) => (
                  <span
                    key={index}
                    className="w-5 h-5 rounded-full inline-block border-[1px] border-solid border-black"
                    style={{ background: color }}
                  ></span>
                ))}
              </div>
              <button 
                className={`mt-2 py-2 w-[200px] rounded flex justify-center gap-2 items-center relative ${
